  ...
  create(socket: DockgeSocket, server: DockgeServer) {
    ...
    socket.on("getTurnstileSiteKey", async (callback) => {
      try {
        ...
        if (!siteKey) {
          log.warn(
            "auth",
            "Turnstile site key is not configured in the environment.",
          );
          callback({
            ok: false,
    ...
    socket.on("login", async (data, callback) => {
      ...
      if (siteKey && secretKey) {
        ...
        log.warn(
          "auth",
          "Turnstile keys are not configured. Skipping CAPTCHA verification.",
        );
      }
    ...
    socket.on("getSettings", async (callback) => {
      try {
        ...
        if (fs.existsSync(path.join(server.stacksDir, "global.env"))) {
          data.globalENV = fs.readFileSync(
            path.join(server.stacksDir, "global.env"),
            "utf-8",
          );
        } else {
