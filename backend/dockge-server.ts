...
export class DockgeServer {
  ...
  constructor() {
    ...
    this.config.sslCert =
      args.sslCert || process.env.DOCKGE_SSL_CERT || undefined;
    this.config.sslKeyPassphrase =
      args.sslKeyPassphrase ||
      process.env.DOCKGE_SSL_KEY_PASSPHRASE ||
      undefined;
    this.config.port = args.port || Number(process.env.DOCKGE_PORT) || 5001;
    this.config.hostname =
      args.hostname || process.env.DOCKGE_HOSTNAME || undefined;
    this.config.dataDir =
      args.dataDir || process.env.DOCKGE_DATA_DIR || "./data/";
    this.config.stacksDir =
      args.stacksDir || process.env.DOCKGE_STACKS_DIR || defaultStacksDir;
    ...
    this.io = new socketIO.Server(this.httpServer, {
      ...
      allowRequest: (req, callback) => {
        ...
        const bypass =
          isDev || process.env.UPTIME_KUMA_WS_ORIGIN_CHECK === "bypass";
          ...
          if (origin) {
            try {
              ...
              if (host !== originURL.host) {
                ...
                log.error(
                  "auth",
                  `Origin (${origin}) does not match host (${host}), IP: ${req.socket.remoteAddress}`,
                );
              }
            } catch (e) {
              ...
              log.error(
                "auth",
                `Invalid origin url (${origin}), IP: ${req.socket.remoteAddress}`,
              );
            }
          } else {
            log.info(
              "auth",
              `Origin is not set, IP: ${req.socket.remoteAddress}`,
            );
          }
        } else {
    ...
    this.io.on("connection", async (socket: Socket) => {
      ...
      if (dockgeSocket.endpoint) {
        log.info(
          "server",
          "Socket connected (agent), as endpoint " + dockgeSocket.endpoint,
        );
      } else {
  ...
  async getClientIP(socket: Socket): Promise<string> {
    ...
    if (await Settings.get("trustProxy")) {
      ...
      if (typeof forwardedFor === "string") {
        ...
        typeof socket.client.conn.request.headers["x-real-ip"] === "string"
      ) {
