...
export class DockerSocketHandler extends AgentSocketHandler {
  create(socket: DockgeSocket, server: DockgeServer, agentSocket: AgentSocket) {
    ...
    agentSocket.on(
      "deleteStack",
      async (name: unknown, deleteOptions: unknown, callback) => {
        try {
          checkLogin(socket);
    ...
    agentSocket.on(
      ...
      async (stackName: unknown, callback) => {
        try {
          ...
          const serviceStatusList = Object.fromEntries(
            await stack.getServiceStatusList(),
          );
          callbackResult(
            {
              ok: true,
    ...
    agentSocket.on(
      "startService",
      async (stackName: unknown, serviceName: unknown, callback) => {
        try {
          checkLogin(socket);
