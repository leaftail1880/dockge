...
export class TerminalSocketHandler extends AgentSocketHandler {
    create(
        ...
        agentSocket.on(
            "terminalInput",
            async (terminalName: unknown, cmd: unknown, callback) => {
                try {
                    checkLogin(socket);
        ...
        agentSocket.on(
            "mainTerminal",
            async (terminalName: unknown, callback) => {
                try {
                    checkLogin(socket);
        ...
        agentSocket.on(
            "attachTerminal",
            async (containerName: unknown, callback) => {
                try {
                    checkLogin(socket);
                    ...
                    // TODO: validate container existence here.

                    const terminalName = `attach-${containerName}-${socket.endpoint}`;

                    const attachTerm =
                        await AttachTerminal.getOrCreateAttachTerminal(
                            server,
                            terminalName,
                            containerName,
                            server.stacksDir,
