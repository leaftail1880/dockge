...
export class Database {
  ...
  static readDBConfig(): DBConfig {
    const dbConfigString = fs
      .readFileSync(path.join(this.server.config.dataDir, "db-config.json"))
      .toString("utf-8");
  ...
  static writeDBConfig(dbConfig: DBConfig) {
    fs.writeFileSync(
      path.join(this.server.config.dataDir, "db-config.json"),
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        if (e.message.includes("the following files are missing:")) {
          ...
          log.warn(
            "db",
            "Database migration failed, you may be downgrading Dockge.",
          );
        } else {
