<template>
  <div>
    ...
    <ul class="list-group">
      <li
        v-for="(networkRow, index) in networkList"
        :key="index"
    ...
    <button class="btn btn-normal btn-sm mt-3 me-2" @click="addField">
    ...
    >
      <input
        :id="'external-network' + index"
        v-model="selectedExternalList[networkName]"
    ...
    <div v-if="false">
      <button class="btn btn-primary btn-sm mt-3 me-2" @click="applyToYAML">
        {{ $t("applyToYAML") }}
      </button>
    </div>
  </div>
