        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              {{ $t("Setup 2FA") }}
              <span v-if="twoFAStatus == true" class="badge bg-primary">{{
                $t("Active")
              }}</span>
              <span v-if="twoFAStatus == false" class="badge bg-primary">{{
                $t("Inactive")
              }}</span>
            </h5>
            <button
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <p v-if="showURI && twoFAStatus == false" class="text-break mt-2">
                {{ uri }}
              </p>
...
export default {
  ...
  methods: {
    ...
    verifyToken() {
      this.$root
        .getSocket()
        .emit("verifyToken", this.token, this.currentPassword, (res) => {
          if (res.ok) {
