...
export default {
  ...
  methods: {
    ...
    removeInput() {
      const textAfterCursorLength =
        this.terminalInputBuffer.length - this.cursorPosition;
      ...
      this.terminal.onKey((e) => {
        ...
        if (e.key === "\r") {
          ...
          if (this.cursorPosition > 0) {
            ...
            const beforeCursor = this.terminalInputBuffer.slice(
              0,
              this.cursorPosition - 1,
            );
            const afterCursor = this.terminalInputBuffer.slice(
              this.cursorPosition,
            );
            ...
            // Redraw the line
            this.terminal.write(
              "\b" + afterCursor + " \b".repeat(afterCursor.length + 1),
            );
          }
        } else if (e.key === "\u001B\u005B\u0033\u007E") {
          // Delete key
          if (this.cursorPosition < this.terminalInputBuffer.length) {
            ...
            this.terminal.write(
              afterCursor + " \b".repeat(afterCursor.length + 1),
            );
          }
        } else if (
          e.key === "\u001B\u005B\u0041" ||
          ...
          // TAB or other special keys
          // Do nothing
        } else {
          // Insert printable character at cursor position
          const textBeforeCursor = this.terminalInputBuffer.slice(
            0,
            this.cursorPosition,
          );
          const textAfterCursor = this.terminalInputBuffer.slice(
            this.cursorPosition,
          );
          this.terminalInputBuffer = textBeforeCursor + e.key + textAfterCursor;
          this.terminal.write(
            e.key + textAfterCursor + "\b".repeat(textAfterCursor.length),
          );
          this.cursorPosition++;
        }
      });
