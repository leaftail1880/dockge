    ...
    <div v-if="$route.name === 'DashboardHome'">
      ...
      <div class="row first-row">
        ...
        <div class="col-md-7">
          ...
          <button class="btn-normal btn mb-4" @click="convertDockerRun">
        ...
        <div class="col-md-5">
          ...
          <div class="shadow-box big-padding">
            <h4 class="mb-3">
              {{ $tc("dockgeAgent", 2) }}
              <span class="badge bg-warning" style="font-size: 12px">beta</span>
            </h4>
            ...
            >
              ...
              <template v-if="$root.agentStatusList[endpoint]">
                <span
                  v-if="$root.agentStatusList[endpoint] === 'online'"
                  class="badge bg-primary me-2"
                  >{{ $t("agentOnline") }}</span
                >
                <span
                  v-else-if="$root.agentStatusList[endpoint] === 'offline'"
              ...
              <template v-if="$root.agentStatusList[endpoint]">
                <span
                  v-if="endpoint === '' && agent.name === ''"
                  class="badge bg-secondary me-2"
                  >Controller</span
                >
                <span
                  v-else-if="agent.name === ''"
                  :href="agent.url"
                  class="me-2"
                  >{{ endpoint }}</span
                >
                <span v-else :href="agent.url" class="me-2">{{
                  agent.name
                }}</span>
              </template>
              ...
              <font-awesome-icon
                icon="pen-to-square"
                @click="
                  showEditAgentNameDialog[agent.name] =
                    !showEditAgentNameDialog[agent.Name]
                "
              />
              ...
              <!-- Edit Dialog -->
              <BModal
                v-model="showEditAgentNameDialog[agent.name]"
                :no-close-on-backdrop="true"
                :close-on-esc="true"
                :okTitle="$t('Update Name')"
                okVariant="info"
                @ok="updateName(agent.url, agent.updatedName)"
              >
                <label for="Update Name" class="form-label"
                  >Current value: {{ $t(agent.name) }}</label
                >
                <input
                  id="updatedName"
                  v-model="agent.updatedName"
                  type="text"
                  class="form-control"
                  optional
                />
