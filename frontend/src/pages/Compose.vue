<template>
  <transition name="slide-fade" appear>
    <div>
      <h2 v-if="isAdd" class="mb-3">{{ $t("compose") }}</h2>
      ...
      <div v-if="stack.isManagedByDockge" class="mb-3">
        ...
        <button
          ...
          class="btn btn-danger"
          :disabled="processing"
          @click="showForceDeleteDialog = !showForceDeleteDialog"
        >
          <font-awesome-icon icon="trash" class="me-1" />
          {{ $t("forceDeleteStack") }}
        </button>
      ...
      <div v-if="stack.isManagedByDockge" class="row">
            ...
            <div class="mt-3">
              ...
              <select v-model="stack.endpoint" class="form-select">
                <option
                  ...
                  :key="endpoint"
                  :value="endpoint"
                  :disabled="$root.agentStatusList[endpoint] != 'online'"
                >
                  ({{ $root.agentStatusList[endpoint] }})
                  {{
                    agent.name !== ""
                      ? agent.name
                      : agent.url || $t("Controller")
                  }}
                </option>
              </select>
            </div>
        ...
        <div
          v-if="
            stack.composeOverrideYAML && stack.composeOverrideYAML.trim() !== ''
          "
        >
          <h4 class="mb-3">
            {{ stack.composeOverrideFileName || "compose.override.yaml" }}
          </h4>
          <div
          ...
          >
            <button
              v-if="isEditMode"
              v-b-modal.compose-override-editor-modal
              class="expand-button"
            >
          ...
          <BModal
            id="compose-override-editor-modal"
            :title="stack.composeOverrideFileName || 'compose.override.yaml'"
            scrollable
            size="fullscreen"
            hide-footer
          >
        ...
        >
          <button
            v-if="isEditMode"
            v-b-modal.compose-editor-modal
            class="expand-button"
          >
            <font-awesome-icon icon="expand" />
          </button>
        ...
        <BModal
          id="compose-editor-modal"
          :title="stack.composeFileName"
          scrollable
          size="fullscreen"
          hide-footer
        >
      ...
      <!-- Force Delete Dialog -->
      <BModal
        v-model="showForceDeleteDialog"
        :okTitle="$t('forceDeleteStack')"
        okVariant="danger"
        @ok="forceDeleteDialog"
      >
  ...
  methods: {
    ...
    deleteDialog() {
      this.$root.emitAgent(
        ...
        "deleteStack",
        this.stack.name,
        { deleteStackFiles: this.deleteStackFiles },
        (res) => {
          this.$root.toastRes(res);
          if (res.ok) {
            this.$router.push("/");
